...
import {
  listAgencyTaTransactions,
  syncAgencyTaTransactions,
  type AgencyBookingDto,
} from '../services/agencyTaTransactionsApi'
...
const rows = ref<AgencyBookingDto[]>([])

const toast = useToast()

// Chỉ Admin/Kế toán được chạy đồng bộ tay (§13); ẩn nút với vai trò khác.
...
// Gộp tiêu đề ngày để dễ đọc theo từng ngày đặt vé.
const displayRows = computed(() => {
  const out: { key: string; isSep: boolean; row: AgencyBookingDto }[] = []
  let lastDate: string | null = null
  for (const row of rows.value) {
    if (row.bookingDate !== lastDate) {
      out.push({ key: `sep-${row.bookingDate}`, isSep: true, row })
      lastDate = row.bookingDate
    }
...
async function load() {
  loading.value = true
  try {
    const result = await listAgencyTaTransactions({
      page: page.value,
...
async function runSync() {
  ...
  syncing.value = true
  try {
    const result = await syncAgencyTaTransactions(syncModal.businessDate)
    const date = formatDate(result.businessDate)
    if (result.matchedParent === 0) {
      toast.info(
        `Không có giao dịch đại lý (mã cấp trên ${result.parentAgencyCode}) cho ngày ${date}.`,
        { duration: 6000 },
      )
    } else {
      let msg = `Đã đồng bộ ngày ${date}: ${result.inserted} thêm mới, ${result.updated} cập nhật, ${result.skipped} không đổi (trên ${result.totalLines} dòng API).`
      if (result.unmatchedAgency > 0) {
        msg += ` ${result.unmatchedAgency} dòng chưa map được đại lý: ${result.unmatchedAgencyCodes.join(', ')}.`
      }
    ...
    toast.error(err instanceof ApiClientError ? err.message : 'Không lấy được giao dịch đại lý trên TA.')
  } finally {
    syncing.value = false
  }
}
...
<template>
  <PageHeader
    ...
    subtitle="Danh sách giao dịch của các đại lý trên hệ thống TA, đồng bộ tự động từ OneInventory"
  />
    ...
    <div v-if="syncModal.open" class="modal-overlay">
      <div class="modal">
        <div class="modal-header">
          <span class="modal-title">Lấy giao dịch đại lý trên TA</span>
        ...
        <div class="modal-body">
          ...
          <div class="notice notice-indigo">
            Giao dịch đại lý (mã cấp trên đã cấu hình) của ngày này sẽ được đồng bộ từ OneInventory. Các
            bản ghi trùng theo ID nguồn sẽ được cập nhật, không tạo trùng.
          </div>
        </div>
      ...
      <table>
        ...
        <tbody>
          ...
          <tr v-for="item in displayRows" :key="item.key" :class="{ 'date-separator': item.isSep }">
            <td v-if="item.isSep" colspan="6">📅 {{ formatDate(item.row.bookingDate) }}</td>
            <template v-else>
              <td class="cell-strong mono">{{ item.row.bookingCode }}</td>
              <td>{{ item.row.buyingAgentName || '-' }}</td>
              <td class="mono">{{ item.row.buyingAgentCode || '-' }}</td>
              <td class="cell-muted">{{ formatDate(item.row.bookingDate) }}</td>
              <td class="amount amount-red" style="text-align: right">{{ formatMoney(item.row.salesAmount) }}</td>
              <td class="td-center">
                <span v-if="item.row.isAgencyMatched" class="badge badge-green">Đã map ĐL</span>
                <span v-else class="badge badge-amber" title="Mã đại lý mua chưa có trong Danh sách đại lý">
                  Chưa map ĐL
                </span>
              </td>
            </template>
          ...
          <tr v-if="!loading && rows.length === 0">
            <td colspan="6" class="cell-muted" style="text-align: center">Chưa có giao dịch đại lý phù hợp.</td>
          </tr>
        </tbody>
      </table>
    </div>
