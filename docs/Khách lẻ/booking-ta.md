...
**Nguồn dữ liệu:** ⚠️ **Demo/mock** — dữ liệu tĩnh trong `data/reports.ts` (`reportPages.retailTaBookings`). Có nút **"⤓ Get API"** giả lập (thuộc danh sách `pagesWithGetApiOnly` trong `AgencyReportView.vue`, cùng logic mock với "Gọi API test" nhưng đặt góc phải toolbar theo style "Danh sách nạp tiền KVC theo ngày").
...
### UC-BKKL-04 – Gọi API test (Get API)
